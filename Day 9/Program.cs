...
namespace ConsoleApplication
{
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            Console.WriteLine($"Decompressed text length: {decompressor.GetDecompressedTextLength(Puzzle.Input)}");
        }
    }
    ...
    public class Decompressor
    {
            ...
            while (index < text.Length)
            {
                ...
                if (isParsingCommand)
                {
                    if (current == ')')
                    {
                        ...
                        var commandParts = command.Split('x');
                        var numberOfCharacters = int.Parse(commandParts[0]);
                        var numberOfRepetitions = int.Parse(commandParts[1]);

                        var blockToRepeat = text.Substring(index + 1, numberOfCharacters);
                        for (var i = 0; i < numberOfRepetitions; i++)
                        {
                            result.Append(blockToRepeat);
                        }
        ...
        public long GetDecompressedTextLength(string text)
        {
            var result = 0L;
            var index = 0;
            var commandBuilder = new StringBuilder();
            ...
            while (index < text.Length)
            {
                ...
                if (isParsingCommand)
                {
                    if (current == ')')
                    {
                        ...
                        for (var i = 0; i < numberOfRepetitions; i++)
                        {
                            subresult.Append(blockToRepeat);
                        }

                        result += GetDecompressedTextLength(subresult.ToString());
