...
namespace ConsoleApplication
{
    ...
    public class Bot
    {
        ...
        public void Receive(Chip chip)
        {
            if (_chips.Count == 2)
            {
                throw new Exception("Bot cannot have more than two chips");
            }
        ...
        public bool IsReady()
        {
            return _chips.Count == 2;
        }

        public void GiveHighTo(Bot bot)
        {
            ThrowIfNotReady();

            var high = _chips.OrderByDescending(c => c.Value).First();
            bot.Receive(high);
            _chips.Remove(high);
        }

        public void GiveLowTo(Bot bot)
        {
            ThrowIfNotReady();

            var low = _chips.OrderBy(c => c.Value).First();
            bot.Receive(low);
            _chips.Remove(low);
        }
        ...
        private void ThrowIfNotReady()
        {
            if (!IsReady())
            {
                throw new Exception("Bot isn't ready, need to have two chips first");
            }
        }
    }
