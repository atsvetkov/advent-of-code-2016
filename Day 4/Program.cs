...
namespace ConsoleApplication
{
    public class Program
    {
        ...
        {
            var scanner = new RoomScanner();
            var lines = Puzzle.Input.Split(new string[] { Environment.NewLine }, StringSplitOptions.RemoveEmptyEntries);

            Console.WriteLine($"The sum of real room sector IDs: {scanner.GetSumOfSectorIDs(lines)}");
        }
    }
    ...
    public class RoomScanner
    {
        ...
        public int GetSumOfSectorIDs(string[] lines)
        {
            foreach (var line in lines)
            {
                Regex regex = new Regex(@"(?<test>[a-z-]*)(?<sectorid>\d+)\[(?<checksum>[a-z]+)\]");
                Match match = regex.Match(line);
                if (match.Success)
                {
                    var roomName = match.Groups["test"].Value;
                    var sectorId = int.Parse(match.Groups["sectorid"].Value);
                    var checksum = match.Groups["checksum"].Value;

                    if (IsValidChecksum(roomName, checksum))
                    {
                        _sum += sectorId;

                        var decryptedName = DecryptRoomName(roomName, sectorId);
                        if (decryptedName.IndexOf("north", StringComparison.OrdinalIgnoreCase) > -1)
                        {
                            Console.WriteLine($"{decryptedName}: {sectorId}");
                        }
                    }
                }
            }
        ...
        private bool IsValidChecksum(string roomName, string checksum)
        {
            var computedChecksum = GenerateChecksum(roomName.Replace("-", ""));
            
            return computedChecksum == checksum;
        }

        private string GenerateChecksum(string name)
        {
            ...
            for (var i = 0; i < name.Length; i++)
            {
                ...
                int count;
                if (!dict.TryGetValue(current, out count))
                {
                    dict[current] = 1;
                    continue;
                }
            ...
            var chars = dict.OrderByDescending(kvp => kvp.Value).ThenBy(kvp => kvp.Key).Take(5).Select(kvp => kvp.Key);

            return new String(chars.ToArray());
        }

        private string DecryptRoomName(string roomName, int sectorId)
        {
            var offset = (int)'a';
            var chars = roomName.TrimEnd('-').ToCharArray();
            for (var i = 0; i < chars.Length; i++)
            {
                ...
                {
                    chars[i] = (char)(((int)c - offset + sectorId) % 26 + offset);
                }
            }
