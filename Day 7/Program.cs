...
namespace ConsoleApplication
{
    public class Program
    {
        ...
        {
            var countTLS = Puzzle.Input.Split(new string[] { Environment.NewLine }, StringSplitOptions.None).Count(AbbaChecker.SupportsTLS);
            Console.WriteLine($"Number of addresses supporting TLS: {countTLS}");

            var countSSL = Puzzle.Input.Split(new string[] { Environment.NewLine }, StringSplitOptions.None).Count(AbbaChecker.SupportsSSL);
            Console.WriteLine($"Number of addresses supporting SSL: {countSSL}");
        }
    }

    public static class AbbaChecker
    {
        public static bool SupportsTLS(string text)
        {
            ...
            for (var i = 0; i <= length - 4; i++)
            {
                ...
                var isAbba = text[i] != text[i + 1] && text[i] == text[i + 3] && text[i + 1] == text[i + 2];
                if (isAbba)
                {
                    if (isInsideBrackets)
                    {
        ...
        public static bool SupportsSSL(string text)
        {
            var length = text.Length;
            var abaOutside = new List<string>();
            var abaInside = new List<string>();
            var isInsideBrackets = false;
            for (var i = 0; i <= length - 3; i++)
            {
                ...
                var isAba = text[i] != text[i + 1] && text[i] == text[i + 2];
                if (isAba)
                {
                    var aba = new String(new [] { text[i], text[i + 1], text[i + 2] });
                    if (isInsideBrackets)
                    {
                        abaInside.Add(aba);
                    }
            ...
            return abaOutside.Any(aba => abaInside.Contains(InvertAba(aba)));
        }

        private static string InvertAba(string text)
        {
            return new String(new [] { text[1], text[0], text[1] });
        }
    }
